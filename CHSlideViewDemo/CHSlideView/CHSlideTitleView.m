...
- (instancetype)initWithFrame:(CGRect)frame forTitles:(NSArray*)titles{
    ...
    if (self) {
        ...
        _widthGap            = frame.size.width/titles.count;
...
- (void)configView{
    ...
    for (NSUInteger i = 0; i<_titles.count; i++) {
        ...
        if (_sizeToFitTitleWidth) {
            ...
            frame.size   = CGSizeMake(_widthGap, CHButtonHeight);
        ...
        NSLog(@"width =%g",button.frame.size.width);
    }
...
- (void)buttonEvents:(UIButton*)button{
    ...
    [currentButton setTitleColor:_tintColor forState:UIControlStateNormal];
    if (_textColor) {
        [currentButton setTitleColor:_textColor forState:UIControlStateNormal];
    }else{
        [currentButton setTitleColor:_tintColor forState:UIControlStateNormal];
    }
    if (_allowedScaleFontSize) {
