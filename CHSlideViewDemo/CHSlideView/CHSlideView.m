...
@interface CHSlideView()<UIScrollViewDelegate>
{
...
- (void)configCellWithCell:(CHSlideCell*)cell forIndex:(NSUInteger)index;
//
- (void)configSlideView;

@end

@implementation CHSlideView
...
#warning 此处是重新的写法，默认是加载两个view。如果想加载一个view 可在次修改
    ...
    NSUInteger currentPage = CGRectGetMinX(mainScrollViewBounds)/SCREEN_WIDTH_CHSLIDE;
    
    NSUInteger nextPage    = CGRectGetMaxX(mainScrollViewBounds)/SCREEN_WIDTH_CHSLIDE;
    
    currentPage            = MAX(currentPage, 0);
    nextPage               = MIN(nextPage, _totaiPageNumber-1);
    
    //回收 unvisible cell
    for (CHSlideCell * cell  in _visibleCells) {
    ...
    for (NSUInteger index = currentPage ; index <= nextPage; index++) {
        ...
        if (![self isVisibleCellForIndex:index]) {
           ...
           CHSlideCell *cell = [_delegate slideView:self cellForRowAtIndex:index];
...
- (CHSlideCell*)dequeueReusableCell{
    ...
    CHSlideCell * cell = [_recycledCells anyObject];
...
- (CHSlideCell*)visibleCellForIndex:(NSUInteger)index{

    CHSlideCell * visibleCell = nil;
...
- (void)configCellWithCell:(CHSlideCell*)cell forIndex:(NSUInteger)index{
    ...
    cellFrame.size.height = cellFrame.size.height - CHSildeTitleViewHeight;
    ...
    //获取偏移量
   __block CHSlideCell *newCell = cell;
...
- (void)reloadData{
    ...
    if ([_delegate respondsToSelector:@selector(columnNumber)]) {
            ...
            if (weakSelf) {
                ...
                [strongSelf.mainScrollview setContentSize:CGSizeMake(CGRectGetWidth(strongSelf.frame)*_totaiPageNumber, CGRectGetHeight(strongSelf.frame)-CHSildeTitleViewHeight)];
...
- (void)configSlideView{
    ...
    self.mainScrollview = ({
        ...
        UIScrollView * scrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, CHSildeTitleViewHeight, CGRectGetWidth(self.frame), CGRectGetHeight(self.frame)-CHSildeTitleViewHeight)];
    ...
    self.slideTitleView = ({
        ...
        CHSlideTitleView * slideTitleView = [[CHSlideTitleView alloc]initWithFrame:slideTitleFrame forTitles:_titles];
...
- (void)setContentViewColor:(UIColor *)contentViewColor{
    ...
    _slideTitleView.backgroundColor = contentViewColor;
}
- (void)setAllowedScrollContentTitle:(BOOL)allowedScrollContentTitle{
    _allowedScrollContentTitle = allowedScrollContentTitle;
    _slideTitleView.scrollEnabled = allowedScrollContentTitle;
}
...
- (void)setTintColor:(UIColor *)tintColor{
    ...
    _slideTitleView.tintColor = tintColor;
    
}
- (void)setWidthGap:(CGFloat)widthGap{
    _widthGap = widthGap;
    _slideTitleView.widthGap = widthGap;
    
}
- (void)setTextColor:(UIColor *)textColor
{
    _textColor = textColor;
    _slideTitleView.textColor = textColor;
}
- (void)setSizeToFitTitleWidth:(BOOL)sizeToFitTitleWidth{
    _sizeToFitTitleWidth = sizeToFitTitleWidth;
    _slideTitleView.sizeToFitTitleWidth = sizeToFitTitleWidth;
}
- (void)setSlideTitleViewTitleMax:(CGFloat)slideTitleViewTitleMax{
    _slideTitleViewTitleMax = slideTitleViewTitleMax;
    _slideTitleView.slideTitleViewTitleMax = slideTitleViewTitleMax;
}
- (void)setSlideTitleViewTitleMin:(CGFloat)slideTitleViewTitleMin{
    _slideTitleViewTitleMin = slideTitleViewTitleMin;
    _slideTitleView.slideTitleViewTitleMin = slideTitleViewTitleMin;
}
- (void)setAllowedScaleFontSize:(BOOL)allowedScaleFontSize{
    _allowedScaleFontSize = allowedScaleFontSize;
    _slideTitleView.allowedScaleFontSize = allowedScaleFontSize;
}
@end
